...
.PHONY: help install checkov-install tflint-install tflint-check tflint-init \
        ...
        test-terratest test-basic test-multiple test-plan test-validate \
...
test-tflint: tflint-init tflint-check ## Run linting tests only
...
test-basic: ## Run basic VLAN test
	@echo "$(BLUE)Running basic VLAN test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkVLANBasic
	@echo "$(GREEN)✓ Basic test completed$(NC)"

test-multiple: ## Run multiple VLANs test
	@echo "$(BLUE)Running multiple VLANs test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkVLANMultiple
	@echo "$(GREEN)✓ Multiple VLANs test completed$(NC)"

test-plan: ## Run plan-only test
	@echo "$(BLUE)Running plan-only test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkVLANPlanOnly
	@echo "$(GREEN)✓ Plan-only test completed$(NC)"

test-validate: ## Run validation test
	@echo "$(BLUE)Running validation test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxNetworkVLANValidation
	@echo "$(GREEN)✓ Validation test completed$(NC)"

test-parallel: ## Run tests in parallel
	@echo "$(BLUE)Running tests in parallel...$(NC)"
	cd terratest && go test -v -timeout 30m -parallel 4
...
terratest-init: ## Initialize terratest Go module
	...
	cd terratest && go mod init github.com/proxmox/modules/proxmox-terraform-network-vlan/terratest || true
...
docs: ## Show module documentation
	@echo "$(BLUE)Proxmox Terraform Network VLAN Module$(NC)"
	@echo ""
	@echo "This module manages Proxmox network VLANs using Terraform or OpenTofu"
	@echo ""
	@echo "Quick Start:"
